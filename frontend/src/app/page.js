      ...
      <nav className="fixed top-0 w-full z-50 glass border-b border-white/40 h-24 flex items-center justify-center">
        <div className="max-w-7xl w-full px-8 md:px-16 flex justify-between items-center">
          ...
          <div className="flex items-center gap-6">
            ...
            <button onClick={() => supabase.auth.signOut()} className="text-[10px] font-black text-gray-400 hover:text-red-700 transition-all uppercase tracking-[0.3em]">Logout</button>
          </div>
        </div>
      </nav>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-white/10 backdrop-blur-md p-6">
          <div className="bg-white/90 max-w-sm w-full p-10 rounded-[2.5rem] premium-shadow relative border border-white">
            ...
            <form onSubmit={handleUpdateProfile} className="space-y-4">
              ...
              <input placeholder="GroupMe or Email" className="w-full bg-gray-50 p-4 rounded-2xl text-sm outline-none mb-4 font-sans" value={contactInfo} onChange={(e) => setContactInfo(e.target.value)}/>
              <button type="submit" className="w-full py-4 bg-[#A51417] text-white text-[10px] font-bold uppercase tracking-widest rounded-full shadow-lg shadow-red-50 hover:bg-black transition-all">Update Profile</button>
              {statusMsg && <p className="text-center text-[10px] font-bold text-red-400 uppercase mt-4">{statusMsg}</p>}
            </form>
          </div>
        </div>
      )}

      {/* MAIN CONTENT - Pushed down to clear header */}
      <div className="max-w-7xl mx-auto px-6 md:px-12 pt-52 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
        ...
        {/* MARKET GRID - 3 columns on large, 2 on medium/mobile */}
        <section className="lg:col-span-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            {offers.length === 0 ? (
                ...
                offers.map((offer) => (
                <div key={offer.id} className="glass p-6 md:p-8 rounded-[2.5rem] premium-shadow card-hover border border-white/60 flex flex-col justify-between aspect-[4/5] md:aspect-auto">
